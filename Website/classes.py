...
engine = create_engine('mysql+pymysql://%s:%s@%s/%s' % (SQL_HOST, SQL_PASSWORD, SQL_IP_ADDRESS, SQL_DATABASE))
...
class Cleanup(Base):
    ...
    #Program Assigned
    host_id = Column(String(250), ForeignKey('users.username'))    #One to Many Relationship: One user to host Many cleanups, part 1 of 2 
...
class User(Base):
    ...
    cleanups_hosted = relationship("Cleanup", backref='host') #One to many relationship, part 2 of 2
